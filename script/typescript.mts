...
function printDiagnostic({ category, code, messageText, file, start }: ts.Diagnostic, additionalHeader?: string): void {
    if (code == 6031) {
        ...
        log.info(`tsc${additionalHeader}`, 'recheck');
    } else if (code == 6194) {
...
function printEmitResult(emitResult: ts.EmitResult): boolean {
    ...
    (emitResult.emitSkipped ? log.error : log.info)('tsc', `completed with ${message}`);
...
class TypeScriptChecker {
    ...
    public check(): TypeScriptResult {
        log.info('tsc', chalk`once {yellow ${this.options.entry}}`);
        setupReadFileHook();
    ...
    public watch(callback: (result: TypeScriptResult) => any) {
        log.info(`tsc${this.additionalHeader}`, chalk`watch {yellow ${this.options.entry}}`);
