...
class MyPacker {
    ...
    private processModule(source: Source, sources: Source[]): Module | null {
        ...
        do {
            ...
            const requiredFileName =
                ...
                : null;
            if (!requiredFileName) {
                log.error(this.logHeader, `${source.filename}: invalid module name ${raw} at ${source.mapIndex(match.index)}`);
                return null;
            }
        ...
        const load = ($module: Module) => {
            for (const request of $module.requests) {
                ...
                if (loadings.some(o => o.moduleName == requestedModule.name)) {  // this also checks self references, but how and why will anyone write self reference?
                    log.error(this.logHeader, 'circular reference encountered');
    ...
    private printResult(assetSize: number, modules: Module[]) {
        log.info(this.logHeader, chalk`completed with {yellow 1} asset {yellow ${filesize(assetSize)}}`);
    ...
    public async run(): Promise<{ success: false } | MyPackResult> {
        ...
        log.info(this.logHeader, this.lastHash ? 'repack' : chalk`pack {yellow ${entry.startsWith('/vbuild/') ? entry.slice(8) : entry}}`);

        const sources = this.getSources();
        if (!sources.some(s => s.filename == entry)) {
            log.error(this.logHeader, 'invalid entry');
        ...
        if (!hasChange) {
            log.info(this.logHeader, chalk`completed with {gray no change}`);
        } else {
